...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        createProductDetailEntriesTable();
        ...
        jdbcTemplate.update("UPDATE products SET status = '销售中' WHERE status IS NULL OR status = ''");
        seedProductsWhenEmpty();
        seedProductCategoriesWhenMissing();
        seedProductImagesWhenMissing();
        seedProductDetailEntriesWhenMissing();
    ...
    private void createProductImagesTable() {
        jdbcTemplate.execute("""
    ...
    private void createHomeSectionsTable() {
        jdbcTemplate.execute("""
    ...
    private void createProductDetailEntriesTable() {
        jdbcTemplate.execute("""
    ...
    private void createHomeSectionItemsTable() {
        jdbcTemplate.execute("""
    ...
    private void seedProductsWhenEmpty() {
        Integer count = jdbcTemplate.queryForObject(
        ...
        jdbcTemplate.batchUpdate("""
            ...
            (name, name_en, price, original_price, image, category, description, tags, rating, sold_count, stock, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, java.util.List.of(
                new Object[]{"暖屿咖啡", "Warm Island Coffee", 38.00, 48.00, "/products/coffee.jpg", "饮品", "精选阿拉比卡豆，暖屿特调配方，每一杯都是温暖的岛屿时光", "热销,招牌", 4.9, 1280, 86, "销售中"},
                new Object[]{"海岛抹茶拿铁", "Island Matcha Latte", 42.00, null, "/products/matcha.jpg", "饮品", "宇治抹茶搭配鲜奶，带来海岛风情的清新体验", "新品", 4.7, 560, 42, "销售中"},
                new Object[]{"椰香芒果慕斯", "Coconut Mango Mousse", 35.00, null, "/products/mousse.jpg", "甜点", "热带芒果与椰奶的完美融合，入口即化的绵密口感", "人气", 4.8, 920, 28, "销售中"},
                new Object[]{"暖屿曲奇礼盒", "Warm Island Cookie Box", 68.00, 88.00, "/products/cookies.jpg", "零食", "手工烘焙黄油曲奇，海岛风味的甜蜜馈赠", "礼盒,热销", 4.9, 2100, 64, "销售中"},
                new Object[]{"海岛冰摇柠檬茶", "Island Lemon Shaken Tea", 28.00, null, "/products/lemon-tea.jpg", "饮品", "新鲜柠檬搭配茉莉花茶，冰摇出夏日海岛的清爽", "清爽", 4.6, 780, 95, "销售中"},
                new Object[]{"海盐焦糖蛋糕", "Sea Salt Caramel Cake", 45.00, null, "/products/cake.jpg", "甜点", "海盐与焦糖的碰撞，每一口都是惊喜的味道层次", "招牌", 4.8, 1050, 33, "销售中"},
                new Object[]{"暖屿帆布袋", "Warm Island Tote Bag", 58.00, null, "/products/tote.jpg", "周边", "原创海岛插画设计，纯棉帆布材质，实用又好看", "周边", 4.7, 650, 58, "已下架"},
                new Object[]{"海岛香薰蜡烛", "Island Scented Candle", 78.00, 98.00, "/products/candle.jpg", "周边", "热带花香与木质调的融合，点燃就是海岛度假的感觉", "新品,礼盒", 4.9, 430, 24, "销售中"}
            ));
    }

    private void seedProductImagesWhenMissing() {
        jdbcTemplate.update("""
    ...
    private void seedProductCategoriesWhenMissing() {
        jdbcTemplate.update("""
            ...
            WHERE NOT EXISTS (
                ...
                WHERE pc.name = source_categories.category
            )
            """);
    }

    private void seedHomeSectionsWhenMissing() {
        jdbcTemplate.update("""
    ...
    private void seedProductDetailEntriesWhenMissing() {
        jdbcTemplate.update("""
        ...
        jdbcTemplate.update("UPDATE product_detail_entries pde JOIN products p ON pde.product_id = p.id SET pde.content = p.category WHERE pde.content = '__CATEGORY__'");
        jdbcTemplate.update("UPDATE product_detail_entries pde SET pde.content = CONCAT('WI-', LPAD(pde.product_id, 5, '0')) WHERE pde.content = '__SKU__'");
        jdbcTemplate.update("UPDATE product_detail_entries pde JOIN products p ON pde.product_id = p.id SET pde.content = CONCAT(p.stock, ' 件') WHERE pde.content = '__STOCK__'");
    }

    private void seedProductVariantsWhenMissing() {
        jdbcTemplate.update("""
    ...
    private void seedHomeSectionItemsWhenMissing() {
        jdbcTemplate.update("""
