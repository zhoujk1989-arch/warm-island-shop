...
<template>
  <div class="min-h-screen pt-20 bg-[#fffaf2]">
    <div v-if="product" class="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 py-10 sm:py-14">
      <div class="mb-8">
      ...
      <div class="grid lg:grid-cols-[1.04fr_0.96fr] gap-10 lg:gap-16 items-start">
        <section>
          <div class="relative aspect-square rounded-[2.5rem] overflow-hidden bg-[radial-gradient(circle_at_32%_18%,#fff8ed_0,#fff8ed00_36%),linear-gradient(135deg,#f8e7d4_0%,#fff5e7_54%,#eccfb6_100%)] border border-[#efd9c6] shadow-2xl shadow-[#d8a06d]/16">
            <div class="absolute inset-6 rounded-[2rem] border border-white/70"></div>
            <div class="absolute top-6 left-6 flex flex-wrap gap-2">
              <span v-for="tag in product.tags" :key="tag" class="px-3 py-1.5 rounded-full bg-white/82 border border-white/70 text-xs font-medium text-[#8a5b3e] shadow-sm">
                {{ tag }}
              </span>
            </div>
          ...
          <div class="grid grid-cols-4 gap-3 sm:gap-4 mt-5">
            <button v-for="(image, index) in galleryImages" :key="`${image.src}-${index}`" @click="selectedImageIndex = index"
              class="relative aspect-square rounded-[1.5rem] bg-white border overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-[#d8a06d]/16"
              ...
              <span class="absolute left-2 bottom-2 px-2 py-1 rounded-full bg-white/82 text-[11px] font-medium text-[#7f6655]">{{ image.label }}</span>
            </button>
          </div>
        </section>
        ...
        <section class="lg:pt-4">
          <div class="flex flex-wrap items-center gap-2 mb-5">
            <span class="px-4 py-2 rounded-full bg-[#fff1df] border border-[#efd9c6] text-sm font-medium text-primary">{{ product.category }}</span>
            <span class="px-4 py-2 rounded-full bg-white/78 border border-[#efd9c6] text-sm text-[#8a7463]">小批量手作</span>
          </div>

          <h1 class="text-3xl sm:text-5xl font-bold text-[#3f3329] leading-tight mb-3">{{ product.name }}</h1>
          ...
          <p class="text-[#756252] leading-8 mb-8 text-base sm:text-lg">{{ product.description }}</p>

          <div class="rounded-[2rem] bg-white/76 border border-[#efd9c6] shadow-sm shadow-[#d8a06d]/10 p-5 sm:p-6 mb-8">
            ...
            <div class="grid sm:grid-cols-3 gap-3 text-sm text-[#7f6655]">
          ...
          <div class="flex flex-col sm:flex-row gap-4">
            <div class="inline-flex items-center justify-between w-full sm:w-36 rounded-full bg-white border border-[#efd9c6] overflow-hidden">
              <button @click="quantity = Math.max(1, quantity - 1)" class="w-11 h-12 flex items-center justify-center text-[#8a7463] hover:text-primary hover:bg-[#fff1df] transition-colors">-</button>
              <span class="font-semibold text-[#4a372b]">{{ quantity }}</span>
              <button @click="quantity += 1" class="w-11 h-12 flex items-center justify-center text-[#8a7463] hover:text-primary hover:bg-[#fff1df] transition-colors">+</button>
            </div>
      ...
      <section v-if="relatedProducts.length" class="mt-16 sm:mt-20">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <router-link v-for="item in relatedProducts" :key="item.id" :to="`/products/${item.id}`" class="group rounded-[2rem] bg-white/78 border border-[#efd9c6] p-5 shadow-sm shadow-[#d8a06d]/10 hover:-translate-y-1 hover:shadow-xl hover:shadow-[#d8a06d]/18 transition-all duration-300">
            <div class="aspect-[1.3] rounded-[1.5rem] bg-[#fff1df] border border-[#f0ddca] mb-4 overflow-hidden group-hover:scale-[1.02] transition-transform">
