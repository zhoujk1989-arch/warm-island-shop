...
<template>
  <div
    class="group bg-[#fffdf8] rounded-[2rem] overflow-hidden border border-[#efd9c6] shadow-sm shadow-[#d8a06d]/10 hover:shadow-xl hover:shadow-[#d8a06d]/20 transition-all duration-300 hover:-translate-y-1.5 cursor-pointer"
    role="link"
    tabindex="0"
  ...
  >
    ...
    <div class="relative aspect-square bg-[radial-gradient(circle_at_35%_25%,#fff8ed_0,#fff8ed00_38%),linear-gradient(135deg,#f9ead8_0%,#fff5e7_55%,#f0d6bd_100%)] overflow-hidden">
      ...
      <div class="absolute inset-5 rounded-[1.6rem] border border-white/55 pointer-events-none"></div>
      <div class="hidden absolute inset-0 items-center justify-center text-6xl">
        {{ productIcon(product) }}
      </div>

      <!-- Tags -->
      <div class="absolute top-3 left-3 z-10 flex flex-wrap gap-1.5 max-w-[calc(100%-1.5rem)]">
        <span v-for="tag in product.tags" :key="tag"
          class="px-3 py-1.5 text-xs font-bold rounded-full backdrop-blur-md border shadow-[0_6px_18px_rgba(63,44,31,0.22)]"
      ...
      <button @click.stop="addToCart(product)"
        class="absolute bottom-4 right-4 w-11 h-11 bg-primary text-white rounded-full shadow-lg shadow-primary/25 flex items-center justify-center opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 hover:bg-primary-dark hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-0.5">
    ...
    <div class="p-5">
      ...
      <p class="text-xs text-[#9b8270] mb-3">{{ product.nameEn }}</p>
      <div class="flex items-center gap-1 mb-3">
        ...
        <span class="text-xs text-[#8d7666] ml-1">{{ product.rating }} ({{ product.soldCount }}已售)</span>
      </div>
